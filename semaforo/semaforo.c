...
int main()
{
  ...
  else if (opcion == 2)
  {
    ...
    {
      if (pid == 0)
        printf("Escribió el hijo\n");
      else
        printf("Escribió el padre\n");
    }
    return 0;
  }
  else
...
semaforo:
  ...
  if (semid < 0)
  { // Si no se pudo crear el arreglo, retorna un valor negativo
    perror("semget");
    exit(1);
  }
  if (semctl(semid, 0, SETVAL, 1) < 0)
  { // semctl inicializa un semaforo, para un arreglo de semaforos (semid), en su posición 0, se va a inicializar
    // el valor del contador (SETVAL) el valor 1.
    perror("semctl");
    exit(1); // Si falla, retorna un valor negativo.
  }
