 ...
 * NOTA: La programación de señales son heredables, es decir, que si se programa una señal en el padre y luego se ejecuta fork, el hijo también
 * tendrá la señal programada.
 * NOTA 2: Las señales programadass tambien se pueden gatillar desde un proceso externo (distinta jerarquia), si se conoce el pid del proceso, desde
...
void cronometro()
{                           // Función ejecutado desde el proceso hijo
  kill(getppid(), SIGUSR1); // La función kill no es una función que solamente mata a un proceso. Se encarga de mandar una señal a un proceso
                            // en particular por su pid. Existen 64 señales donde 2 son destinados a que el programador decida que hacer con ellos
...
void incrementar()
{ // Función ejecutado desde el proceso padre
  seg++;
}
...
int main()
{
  ...
  {                              // En el hijo...
    ...
    alarm(1);                    // Función que programa la ejecución de la señal SIGALRM cada 1 segundo.
    while (1);
  }
  else
  {                               // En el padre...
    ...
    { // Verifica si al hijo fue señalizado de alguna forma.
